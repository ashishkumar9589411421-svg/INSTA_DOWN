...
# Determine storage paths (Render Disk -> /data, Local -> BASE_DIR)
# This allows persistence if a Disk is attached on Render.
...
SECRET_FILE = os.path.join(STORAGE_DIR, "secret.key")
...
def get_db_connection():
    # check_same_thread=False is crucial for ThreadPoolExecutor compatibility
...
def get_status():
    ...
    maintenance = conn.execute("SELECT value FROM settings WHERE key='maintenance'").fetchone()[0]
    announcement = conn.execute("SELECT value FROM settings WHERE key='announcement'").fetchone()[0]
    
    username, is_admin, plan = "", False, "Guest"
    if user_id:
        row = conn.execute("SELECT username, is_admin, plan FROM users WHERE id=?", (user_id,)).fetchone()
        if row: username, is_admin, plan = row[0], (row[1] == 1), row[2]
    conn.close()
...
def manage_settings():
    ...
    m = conn.execute("SELECT value FROM settings WHERE key='maintenance'").fetchone()[0]
    a = conn.execute("SELECT value FROM settings WHERE key='announcement'").fetchone()[0]
    conn.close()
    return jsonify({"maintenance": m == 'true', "announcement": a})
...
def serve_up(filename):
    # Checks token in Header OR Query Param
...
def get_video_formats(url):
    ydl_opts = { "quiet": True, "no_warnings": True, "noplaylist": True, "extract_flat": "in_playlist", "http_headers": { "User-Agent": "Mozilla/5.0" }, "cachedir": False }
...
def process_download(job_id, url, fmt_id):
    with download_semaphore:
        ...
        ydl_opts = {
            ...
            "quiet": True, "cachedir": False, "format": "best"
        }
        if FFMPEG_PATH: ydl_opts["ffmpeg_location"] = FFMPEG_PATH
        if "audio" in fmt_id: 
        ...
        except Exception as e: job_status[job_id].update({"status": "error", "error": str(e)})
...
def api_download():
    ...
    if get_setting('maintenance') == 'true' and not is_admin_request(request):
