...
# FIX: Allow token via Query Param for Images
...
def manage_settings():
    ...
    conn.close()
    return jsonify({"maintenance": m == 'true', "announcement": a, "message": locals().get('msg', '')})
...
def serve_up(filename):
    # Fix: This will now use query param token if header is missing
